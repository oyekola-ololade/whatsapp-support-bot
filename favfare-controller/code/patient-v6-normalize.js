...
const sessionId = (str('session_id') || `demo-${Date.now()}`)
...
let history = [];
try {
  history = JSON.parse(String(b.history ?? '[]'));
...
if (action === 'chat') {
  ...
  api_url = $env.FAVFARE_DATA_GATEWAY_URL;
  payload = {
    action: 'quote_and_capture',
    service_slug: serviceSlug,
    message: message || `I am interested in ${serviceName}`,
    full_name: 'WhatsApp Patient',
    phone: '+2348000000200',
    whatsapp_id: `favfare-${sessionId}`,
    session_id: sessionId,
  };
} else if (action === 'capture_details') {
  api_url = $env.FAVFARE_DATA_GATEWAY_URL;
  payload = {
    action: 'save_details_and_calendar',
