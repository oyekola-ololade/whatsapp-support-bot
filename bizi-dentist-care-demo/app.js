...
async function api(path,body){const r=await fetch(path,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});const d=await r.json();if(!r.ok||d?.ok===false)throw new Error(d?.error||'Demo request failed');return d}
...
let demo=null,level=2,remote='',messages=[],pendingChoices=[],handoff=false,lastEnquiryId=null,p1={stage:'idle',name:'',email:'',date:'',time:''};
const outcome={
...
function showBuilder(){
  $('#builder').classList.remove('hidden');$('#experience').classList.add('hidden');
...
function applyDemo(){
  const c=demo.client,brand=c.branding||{},name=c.display_name,location=brand.location||c.metadata?.location||'Your clinic';
  ...
  $('#clinicTitle').textContent=name;$('#clinicSub').textContent=`${location} · your live demo`;$('#phoneClinic').textContent=name;
  ...
  $('#storyTitle').textContent=outcome[level].story;$('#storyCopy').textContent=level===1?'Routine questions are handled. Appointment preferences become requests for staff to confirm. Clinical judgement is escalated.':level===2?'Routine questions, booking state and follow-up can move automatically, while staff can take control at any point.':'Website and approved channels can feed the same operational flow, while staff keeps one place to manage the work.';
  ...
  remote=sessionStorage.getItem(`bizi-remote-${c.client_key}`)||`234800${String(Math.floor(Math.random()*9000000)+1000000)}@s.whatsapp.net`;sessionStorage.setItem(`bizi-remote-${c.client_key}`,remote);
  resetChat();renderWebsite();
}

function chatBubble(who,text){messages.push({who,text});renderThread()}
function renderThread(typing=false){
  const th=$('#thread');th.innerHTML=messages.map(m=>`<div class="msg ${esc(m.who)}"><div>${esc(m.text)}<small>${m.who==='user'?'You':m.who==='staff'?'Clinic staff':'Patient Assistant'} · now</small></div></div>`).join('')+(typing?'<div class="msg assistant"><div class="typing"><i></i><i></i><i></i></div></div>':'');th.scrollTop=th.scrollHeight;renderQuick();renderState();
}
function renderQuick(){
  const q=$('#quickReplies');if(handoff){q.innerHTML='';return}
  if(pendingChoices.length){q.innerHTML=pendingChoices.map((c,i)=>`<button type="button" data-choice="${i}">${esc(c.label||c.date||c.time||`Option ${i+1}`)}</button>`).join('');$$('[data-choice]',q).forEach(b=>b.onclick=()=>sendChat(String(Number(b.dataset.choice)+1)));return}
...
function renderState(){
  const svc=demo?.services?.[0]?.name||'Featured service';const rows=[['Care level',`Care ${level}`],['Featured service',svc],['Conversation',handoff?'Human takeover':'Assistant active'],['Staff record',lastEnquiryId?'Created / updated':'Ready to capture']];
  $('#stateRows').innerHTML=rows.map(r=>`<div class="state-row"><span>${esc(r[0])}</span><b>${esc(r[1])}</b></div>`).join('');$('#automationState').textContent=handoff?'Human takeover':'Assistant active';$('#automationState').classList.toggle('human',handoff);
}
function resetChat(){
  const name=demo.client.display_name;messages=[{who:'assistant',text:`Hi 👋 Welcome to ${name}. How can I help you today?`}];pendingChoices=[];handoff=false;lastEnquiryId=null;p1={stage:'idle',name:'',email:'',date:'',time:''};renderThread();
}
...
async function packageOneRequest(text){
  ...
  if(p1.stage==='idle'){p1.stage='name';chatBubble('assistant',`Sure. What name should I put on the ${demo.services?.[0]?.name||'appointment'} request?`);return true}
  if(p1.stage==='name'){if(text.length<2||/yes|no|what|why|hello/i.test(text)){chatBubble('assistant','What name should I put on the request?');return true}p1.name=text;p1.stage='email';chatBubble('assistant','Thanks. What email should the clinic use if they need to confirm the request?');return true}
  if(p1.stage==='email'){if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(text)){chatBubble('assistant','Send me a valid email address for the request.');return true}p1.email=text;p1.stage='date';chatBubble('assistant','What day would you prefer?');return true}
  if(p1.stage==='date'){p1.date=text;p1.stage='time';chatBubble('assistant','What time or time window works best for you?');return true}
  if(p1.stage==='time'){
...
async function sendChat(raw){
  ...
  chatBubble('user',text);pendingChoices=[];renderThread(true);
  try{
    const d=await api('/api/chat',{client_key:demo.client.client_key,channel_id:demo.channel_id,remote_jid:remote,message:text,message_id:`webdemo-${Date.now()}-${Math.random().toString(16).slice(2)}`});lastEnquiryId=d.enquiry_id||lastEnquiryId;
    if(/need a person|talk to staff|receptionist|human/i.test(text)&&lastEnquiryId){try{await api('/api/action',{client_key:demo.client.client_key,enquiry_id:lastEnquiryId,action_type:'take_over',staff_id:demo.staff?.id});handoff=true;messages.push({who:'assistant',text:'Of course. I’ve handed this conversation to the clinic team.'},{who:'staff',text:'A staff member has taken over this demo conversation.'});renderThread();return}catch{}}
    if(d.reply)messages.push({who:'assistant',text:d.reply});if(d.handoff||d.suppressed){handoff=true;if(d.handoff)messages.push({who:'staff',text:'This conversation is now waiting for clinic staff.'})}pendingChoices=Array.isArray(d.choices)?d.choices:[];renderThread();
...
function renderStaff(rows){
  const root=$('#staffSurface'),m=metrics(rows);if(level===1){root.innerHTML=`<div class="staff-surface simple-sheet"><div class="outcome-card"><strong>No full CRM to learn.</strong><p>Your team still works mainly in WhatsApp. This table exists so names, service interest, appointment requests and human-attention flags do not get lost.</p></div><div class="simple-sheet-note">The patient conversation you just tried can create a real synthetic row here. Appointment requests remain requests until staff confirms them.</div>${tableCard(rows,false)}</div>`;wireRows();return}
...
async function submitWebsite(e){e.preventDefault();const btn=$('button',e.currentTarget);btn.disabled=true;btn.textContent='Sending…';try{await api('/api/website',{client_key:demo.client.client_key,full_name:$('#webName').value,email:$('#webEmail').value,message:$('#webMessage').value,service_slug:$('#webService').value});toast('Website enquiry captured. Open the staff view to see it.');e.currentTarget.reset();}catch(err){toast(err.message)}finally{btn.disabled=false;btn.textContent='Send enquiry'}}

async function createDemo(e){
  e.preventDefault();const btn=$('#buildBtn'),selected=Number($('input[name="level"]:checked').value);btn.disabled=true;btn.querySelector('span').textContent='Building your clinic…';try{const d=await api('/api/create',{clinic_name:$('#clinicName').value,location:$('#location').value,brand_color:$('#brandColor').value,featured_service:$('#serviceName').value,featured_price:$('#servicePrice').value,package_level:selected});history.replaceState(null,'',`${location.pathname}?demo=${encodeURIComponent(d.client_key)}`);await loadDemo(d.client_key)}catch(err){toast(err.message)}finally{btn.disabled=false;btn.querySelector('span').textContent='Show me my clinic'}}
async function loadDemo(key){
  try{demo=await api('/api/config',{client_key:key});$('#builder').classList.add('hidden');$('#experience').classList.remove('hidden');applyDemo();switchView('patient');window.scrollTo({top:0});}catch(e){toast('That demo has expired. Build a fresh one.');showBuilder()}
}

$('#builderForm').addEventListener('submit',createDemo);$('#brandColor').addEventListener('input',e=>{$('#colorText').textContent=e.target.value.toUpperCase();setBrand(e.target.value)});$$('input[name="level"]').forEach(r=>r.onchange=()=>$$('.level-option').forEach(x=>x.classList.toggle('selected',$('input',x).checked)));$('#chatForm').onsubmit=e=>{e.preventDefault();sendChat($('#chatInput').value)};$('#openStaff').onclick=()=>switchView('staff');$('#refreshStaff').onclick=refreshStaff;$('#tryAnother').onclick=showBuilder;$$('[data-view]').forEach(b=>b.onclick=()=>switchView(b.dataset.view));
