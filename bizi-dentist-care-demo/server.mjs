...
async function api(req,res,u){
  ...
  else return send(res,404,{ok:false,error:'not_found'});return send(res,r.status,r.data);
}
const server=http.createServer(async(req,res)=>{try{const u=new URL(req.url||'/','http://localhost');if(req.method==='GET'&&u.pathname==='/health')return send(res,200,{ok:true,service:'bizi-dentist-live-demo',version:4,core_backed:true});if(u.pathname.startsWith('/api/')){if(req.method!=='POST')return send(res,405,{ok:false,error:'method_not_allowed'});return api(req,res,u)}if(req.method!=='GET'&&req.method!=='HEAD')return send(res,405,{ok:false,error:'method_not_allowed'});const entry=files[u.pathname];if(!entry)return send(res,404,{ok:false,error:'not_found'});const [name,type]=entry,body=await fs.readFile(path.join(__dirname,name));res.writeHead(200,{'content-type':type,...security});if(req.method==='HEAD')return res.end();res.end(body)}catch(e){console.error('LIVE_DEMO_ERROR',e?.message||e);send(res,Number(e?.status)||500,{ok:false,error:e?.message||'internal_error'})}});
server.listen(PORT,'0.0.0.0',()=>console.log('BIZI_DENTIST_LIVE_DEMO_READY',PORT));

async function selfTest(){if(!RUN_SELF_TEST)return;try{const created=await core('bizi-demo-generator',{action:'create_demo',clinic_name:'Outcome Dental Test',location:'Demo City',brand_color:'#245f9d',featured_service:'Dental Cleaning',featured_price:'₦25,000',package_level:2});if(!created.ok||!created.data?.client_key)throw new Error(`create:${created.status}:${created.data?.error||'failed'}`);const k=created.data.client_key,ch=created.data.channel_id;const chat=await core('bizi-core-whatsapp',{action:'process_inbound',client_key:k,channel_id:ch,remote_jid:'2348000009191@s.whatsapp.net',message:'How much is Dental Cleaning?',message_id:`demo-selftest-${Date.now()}`,push_name:'Demo Patient',is_demo:true});const crm=await core('bizi-core-crm',{action:'list_enquiries',client_key:k,is_demo:true,limit:20});console.log('DEMO_SELF_TEST',JSON.stringify({created:created.ok,chat_status:chat.status,chat_http_ok:chat.ok,chat_ok:chat.data?.ok===true,chat_error:chat.data?.error||null,chat_reason:chat.data?.reason||null,reply:chat.data?.reply||null,crm_status:crm.status,crm:crm.ok&&Array.isArray(crm.data?.enquiries),enquiries:crm.data?.enquiries?.length||0,client_key:k}))}catch(e){console.error('DEMO_SELF_TEST_FAILED',e?.message||e)}}
